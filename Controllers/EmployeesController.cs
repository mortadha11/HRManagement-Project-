...
public class EmployeesController : ControllerBase
{
    ...
    public EmployeesController(AppDbContext context, CredentialGeneratorService credentials)
    {
        _context = context;
    ...
    {
        var validation = await ValidateEmployeeRequest(request.FirstName, request.LastName, request.Email, request.ManagerId, null);
    ...
    public async Task<IActionResult> Update(int id, [FromBody] UpdateEmployeeRequest request)
    {
        ...
        var validation = await ValidateEmployeeRequest(request.FirstName, request.LastName, request.Email, request.ManagerId, id);
        if (validation != null) return validation;
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        if (employee.Subordinates.Any(e => e.IsActive))
            return Conflict(new { message = "Reassign active subordinates before deleting this employee." });
    ...
    public async Task<IActionResult> ResetPassword(int id, [FromBody] ResetEmployeePasswordRequest request)
    {
        ...
        var useManualPassword = !string.IsNullOrWhiteSpace(request.NewPassword);
        var temporaryPassword = useManualPassword
            ? request.NewPassword!.Trim()
            : _credentials.GenerateTemporaryPassword();

        if (useManualPassword && temporaryPassword.Length < 6)
            return BadRequest(new { message = "The password must contain at least 6 characters." });
    ...
    private async Task<IActionResult?> ValidateEmployeeRequest(string firstName, string lastName, string email, int? managerId, int? employeeId)
    {
        ...
        if (managerId.HasValue && !await _context.Employees.AnyAsync(e => e.Id == managerId.Value && e.IsActive))
    ...
    private bool CanAccessEmployee(int employeeId)
    {
        ...
        return int.TryParse(employeeIdClaim, out var currentEmployeeId) && currentEmployeeId == employeeId;
    }
